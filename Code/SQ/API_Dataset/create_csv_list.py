...
class create_csv_list():
    ...
    def get_annotation_set(self, API_TOKEN, URL,  id_list, HERE):
        ...
        for id in id_list: 
            annotation_url = '/' + str(id) + '/export?template=dataframe.csv&disposition=attachment&include_columns=["label.id","label.uuid","label.name","needs_review","tag_names","point.id","point.x","point.y","point.t","point.data","point.media.id","point.media.key","point.media.path_best","point.pose.timestamp","point.pose.lat","point.pose.lon","point.pose.alt","point.pose.dep","label.translated.id","label.translated.uuid","label.translated.name","label.translated.lineage_names","label.translated.translation_info"]&f={"operations":[{"module":"pandas","method":"json_normalize"},{"method":"sort_index","kwargs":{"axis":1}}]}&q={"filters":[{"name":"label_id","op":"is_not_null"}]}&translate={"vocab_registry_keys":["worms","caab","catami"],"target_label_scheme_id":null}'
            ...
            counter += 1
            print('CSV file number {} saved for id: {}'.format(counter, id))
        return NAME
